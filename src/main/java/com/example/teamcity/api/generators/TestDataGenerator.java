        ...
        try{
            ...
            for (var field : testData.getClass().getDeclaredFields()) {
                ...
                if (BaseModel.class.isAssignableFrom(field.getType())) {
                    ...
                    field.set(testData, generatedModel);
                    generatedModels.add(generatedModel);
                }
                field.setAccessible(false);
            }
            return  testData;
        } catch (InvocationTargetException | RuntimeException | InstantiationException | IllegalAccessException | NoSuchMethodException e) {
            throw new IllegalStateException("Cannot generate test data.", e);
        }
    }
}